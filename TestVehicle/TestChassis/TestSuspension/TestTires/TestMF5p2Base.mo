within TestVehicle.TestChassis.TestSuspension.TestTires;
...
model TestMF5p2Base
  ...
  Modelica.Blocks.Sources.RealExpression realExpression[3](y = {0, 1200, 0}) annotation(
  ...
  Modelica.Mechanics.MultiBody.Joints.Prismatic free_y(n = {0, 0, 1}) annotation(
...
equation
  ...
  connect(forceAndTorque.frame_b, MF5p2Base.frame_b) annotation(
    Line(points = {{0, 40}, {20, 40}}, color = {95, 95, 95}));
  connect(realExpression.y, forceAndTorque.torque) annotation(
    Line(points = {{-38, 40}, {-30, 40}, {-30, 46}, {-22, 46}}, color = {0, 0, 127}, thickness = 0.5));
  connect(realExpression.y, forceAndTorque.force) annotation(
    Line(points = {{-38, 40}, {-30, 40}, {-30, 34}, {-22, 34}}, color = {0, 0, 127}, thickness = 0.5));
  connect(fixed.frame_b, free_y.frame_a) annotation(
    Line(points = {{30, -80}, {-30, -80}, {-30, -60}}, color = {95, 95, 95}));
  connect(free_y.frame_b, MF5p2Base.frame_b) annotation(
    Line(points = {{-30, -40}, {-30, 20}, {20, 20}, {20, 40}}, color = {95, 95, 95}));
  annotation(
